...
public class ChallengeAttemptControllerTest {
	...
	public void postValidResult() throws Exception {
		...
		MockHttpServletResponse response = mvc.perform(
				post("/attempts").contentType(MediaType.APPLICATION_JSON)
						.content(jsonRequestAttempt.write(attemptDTO).getJson())) //jsonRequestAttempt.write(attemptDTO).getJson()
				.andDo(MockMvcResultHandlers.print())
				.andReturn().getResponse();
		// then
		then(response.getStatus()).isEqualTo(HttpStatus.OK.value());
		then(response.getContentAsString()).isEqualTo(jsonResultAttempt.write(expectedResponse).getJson()); //response.getContentAsString()
	}
	...
	public void getUserStats() throws Exception {
		...
		// when
		MockHttpServletResponse response = mvc.perform(get("/attempts").param("alias", "john_doe"))
				.andReturn().getResponse();
